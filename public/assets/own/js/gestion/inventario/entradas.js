...
var btn_agregar = '#agregar_entradas';
// Botón quitar entradas
var btn_quitar = '#quitar_entradas';
...
$(function(){
    ...
    /**************** Clonación de un tr *************************/
    var tr          = $("#dataTable tbody#entradas tr.datos_entradas:eq(0)").clone();
    var tbody       = "#dataTable tbody#entradas";
    var tr_alterar  = "#dataTable tbody#entradas tr.datos_entradas";
    ...
    $(document).on('click', btn_quitar, function(){
        if($("input:checkbox.chk_eliminar:checked").length == 0){
            ...
            var chk_marcados = $( "input:checkbox.chk_eliminar:checked" );

            $.each(chk_marcados, function(i, e){
                $(e).parent().parent().remove();
            });
            var ultimo_tr_remover = $(tr_alterar+":last").children().children().attr('data-id');
            // Reseteamos la suma
    ...
    $(document).on('change', '.slc_producto', function(){
        ...
        var suma_total_prec_tot = 0;
        var num_elem = Number($('tr.datos_entradas td label.lbl_verPrecTot').length) - 1;
    ...
    $(document).on('keyup', '.inventario', function(){
        var input_orden = $(this).attr('data-id');
...
/****** Función para clonar una fila, y alterar sus elementos ******/
...
function suma_exist_prec(input_orden){
    var precio_orden = ($('#precio'+input_orden).val() == '') ? '0' : $('#precio' + input_orden).val();
    var entrad_orden = ($('#entrada'+input_orden).val() == '') ? '0' : $('#entrada' + input_orden).val();
    var cant_exist_orden = $('#lbl_Existencia' + input_orden).html();
    var prectot_orden_act = $('#lbl_PrecTot' + input_orden).html();

    var sum_exist_nueva = Number(entrad_orden) + Number(cant_exist_orden);
    var prec_tot_nuevo = roundToTwo(Number(sum_exist_nueva) * parseFloat(precio_orden));
...
function NumeroEntero(number){
    return !(number < 1 || number > 99999999 || number != parseInt(number));
}
