...
$(function(){
	...
	$('.dt-inv').click(function(){
		...
		dataOrden = $(this).attr('data-orden');
		nombreProducto = $(this).parent().parent().find('td:eq(1)').html();
		...
		if(dataOrden == 1){
			...
			//$('#mensaje_inicio').fadeOut(8000, 'linear', listar_detalle_stock(dataID));
	...
	$(document).on('dblclick', '.editar_prec', function(){
		...
		//alert(($(this).index() + ' orden: '+$(this).attr('data-orden') + 'mod2: '+$(this).index()%2));return false;
	...
	$(document).on('click', '.cancelar_det', function(){
		// alert($(this).parent().prev().children().attr('data-valor'));
		var camposCancelar = $(this).attr('data-edit');//1: solo primerCampo 2: amboscampos 3: solo segundoCampo
		if(camposCancelar == 2){
	...
	$(document).on('click', '.guardar_det', function(){
		...
		var camposActualizar = $(this).attr('data-edit');//1: solo primerCampo 2: amboscampos 3: solo segundoCampo
